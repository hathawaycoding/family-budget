...
import type { AuditEvent, BillInstance, BudgetMonth, DebtAccount, IncomeEntry, Note, PlannedExpense, SavingsActivity, SavingsFund, SpendingCategory, SpendingTransaction } from "@/lib/types";
...
export async function getBudgetData() {
  ...
  const [monthRows, incomeRows, billRows, categoryRows, transactionRows, plannedRows, savingsFundRows, savingsActivityRows, debtRows, noteRows, auditRows] = await Promise.all([
  ...
  const debtAccounts: DebtAccount[] = debtRows.map((debt) => ({ id: debt.id, name: debt.name, currentBalanceCents: debt.currentBalanceCents, interestRatePercent: Number(debt.interestRatePercent), minimumPaymentCents: debt.minimumPaymentCents, dueDay: debt.dueDay, extraPaymentCents: debt.extraPaymentCents }));
  ...
  return { household, months, income, bills, categories, transactions, plannedExpenses, savingsFunds, savingsActivities, debtAccounts, notes, auditEvents };
}
