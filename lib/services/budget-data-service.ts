...
import type { AuditEvent, BillInstance, BudgetMonth, DebtAccount, FutureExpense, IncomeEntry, Note, PlannedExpense, SavingsActivity, SavingsFund, ShoppingCheck, ShoppingCheckStatus, SpendingCategory, SpendingTransaction } from "@/lib/types";
...
function effectiveShoppingStatus(status: ShoppingCheckStatus, purchaseDate: string, today = new Date().toISOString().slice(0, 10)): ShoppingCheckStatus {
  if ((status === "DRAFT" || status === "PENDING_APPROVAL") && purchaseDate < today) return "EXPIRED";
  return status;
}
...
export async function getBudgetData() {
  ...
  const [monthRows, incomeRows, billRows, categoryRows, transactionRows, shoppingCheckRows, plannedRows, futureExpenseRows, savingsFundRows, savingsActivityRows, debtRows, noteRows, auditRows] = await Promise.all([
    ...
    prisma.shoppingCheck.findMany({ where: { householdId: household.id }, include: { requestedBy: true, reviewedBy: true }, orderBy: [{ createdAt: "desc" }] }),
  ...
  const shoppingChecks: ShoppingCheck[] = shoppingCheckRows.map((check) => {
    const purchaseDate = dateOnly(check.date);
    return { id: check.id, monthId: monthRowsByDbId.get(check.budgetMonthId)?.id ?? "", date: purchaseDate, merchant: check.merchant, categoryId: check.categoryId, amountCents: check.amountCents, cashFlowTreatment: check.cashFlowTreatment, status: effectiveShoppingStatus(check.status, purchaseDate), requestedBy: check.requestedBy.label, reviewedBy: check.reviewedBy?.label ?? null, reviewedAt: check.reviewedAt?.toISOString() ?? null, requestNote: check.requestNote, reviewResponseNote: check.reviewResponseNote, convertedTransactionId: check.convertedTransactionId, createdAt: check.createdAt.toISOString() };
  });
  ...
  return { household, months, income, bills, categories, transactions, shoppingChecks, plannedExpenses, futureExpenses, savingsFunds, savingsActivities, debtAccounts, notes, auditEvents };
}
...
export async function getMonthBundle(monthId: string) {
  ...
  const monthShoppingChecks = data.shoppingChecks.filter((item) => item.monthId === month.id);
  ...
  return { ...data, month, income: monthIncome, bills: monthBills, transactions: monthTransactions, shoppingChecks: monthShoppingChecks, plannedExpenses: monthPlanned, futureExpenses: monthFutureExpenses, savingsActivities: monthSavings, cashFlowRows, previewCashFlowRows, summary };
}
