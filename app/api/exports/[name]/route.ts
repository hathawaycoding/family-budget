...
export async function GET(_: Request, { params }: { params: Promise<{ name: string }> }) {
  ...
  const { auditEvents, bills, debtAccounts, futureExpenses, savingsFunds, transactions } = data;
  const rows: Record<string, string | number | boolean | null | undefined>[] = key === "transactions" ? transactions.map((tx) => ({ date: tx.date, merchant: tx.merchant, amountCents: tx.totalAmountCents, treatment: tx.cashFlowTreatment }))
    ...
    : key === "future-expenses" ? futureExpenses.map((expense) => ({ dueDate: expense.dueDate, description: expense.description, expectedAmountCents: expense.expectedAmountCents, categoryId: expense.categoryId, priority: expense.priority, status: expense.status, includeInPlanPreview: expense.includeInPlanPreview }))
